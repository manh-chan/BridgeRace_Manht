...
public class Cache 
{
    ...
    private static Dictionary<Collider, Gate> gate = new Dictionary<Collider, Gate>();

    public static Gate GetGate(Collider collider)
    {
        if (!gate.ContainsKey(collider))
        {
            gate.Add(collider, collider.GetComponent<Gate>());
        }
